...
export default function EditCustomerDialog({ customer, setCustomers }: EditCustomerDialogProps) {
    ...
    try {
      const response = await fetch(`/api/customer`, {
        ...
        body: JSON.stringify({
                phone_number:customer.phone_number,
                name:formData.name,
                new_phone_number:formData.phone_number,
      ...
      if (response.ok) {
        setCustomers(prevCustomers =>
          prevCustomers.map(c =>
            c.name === customer.name ? { ...c, ...formData } : c
          )
        );
        setIsOpen(false);
      } else {
      ...
      <div className="bg-darkgray rounded-lg w-full max-w-[95%] sm:max-w-md md:max-w-lg border border-lighterGray max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-darkgray border-b border-lighterGray px-3 py-2 sm:px-4 sm:py-3 md:px-6 md:py-4">
          <div className="flex justify-between items-center">
            <h2 className="text-base sm:text-lg md:text-xl text-gray-100">تعديل بيانات العميل</h2>
            <button 
        ...
        <div className="space-y-1 sm:space-y-2">
            ...
            <input
              ...
              className={"w-full p-1.5 sm:p-2 rounded text-sm sm:text-base bg-realyDarkGray text-paleGray border border-lighterGray focus:border-paleOrange outline-none"}/>
          </div>
          <div className="space-y-1 sm:space-y-2">
